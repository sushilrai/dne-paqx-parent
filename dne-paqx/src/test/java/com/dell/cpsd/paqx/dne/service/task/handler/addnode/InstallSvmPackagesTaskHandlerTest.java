...
/**
 * The tests for the InstallSvmPackagesTaskHandler class.
 * <p>
 * Copyright &copy; 2017 Dell Inc. or its subsidiaries. All Rights Reserved. Dell EMC Confidential/Proprietary Information
 * </p>
 *
...
public class InstallSvmPackagesTaskHandlerTest
{
    @Mock
    private Job job;
    ...
    private InstallSvmPackagesTaskHandler handler;
    private String scaleIoSvmManagementIpAddress = "1.2.3.4";

    @Before
    public void setUp()
    {
        this.handler = spy(new InstallSvmPackagesTaskHandler(this.nodeService, this.repository));
    }

    @Test
    public void executeTask_should_successfully_request_the_svm_packages_installation() throws Exception
    {
    ...
    @Test
    public void executeTask_should_fail_the_work_flow_if_the_install_svm_packages_request_fails() throws Exception
    {
        doReturn(this.taskResponse).when(this.handler).initializeResponse(this.job);
        doReturn(this.componentEndpointIds).when(this.repository).getComponentEndpointIds(anyString(), anyString(), anyString());
        doReturn(this.nodeExpansionRequest).when(this.job).getInputParams();
